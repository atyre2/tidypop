...
hh_wide <- hh_wide %>% mutate(S_1991 = lead(`1992`) / `1991`,
...
hh_fecund <- hh_wide %>% # start with census data
  ...
  summarize(numerator = first(N), # this is the number of new recruits in that year
            denominator = sum(N[-1])) %>% # this is the number of adults in that year
