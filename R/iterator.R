iterate_pop <- function(popfun = NULL, parms = list()){
  if (is.null(popfun)) stop("must provide population function")
  if (length(parms) == 0) stop("must provide parameter list")
  N <- vector("numeric", length = length(parms$t))
  N[1] <- parms$N0  # first entry in vector is initial population size
  last_t <- length(N)
